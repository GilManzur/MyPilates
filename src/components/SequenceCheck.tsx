...
export function SequenceCheck({
  ...
  return (
    <div className="report-sheet" role="dialog" aria-modal="true" aria-label="בדיקת רצף מסמכים">
          ...
          <section key={report.key} className="seq-card">
            <div className="seq-card__head">
              ...
              {report.last === 0 ? (
                ...
                <span className="seq-pill seq-pill--ok">
                  רצף תקין · {formatDocumentNumber(report.sampleType, 1)}–
            ...
            {report.missing.length > 0 && (
              <p className="seq-card__missing">
                חסרים:{' '}
                {report.missing.map((n) => formatDocumentNumber(report.sampleType, n)).join(', ')}
              </p>
            )}
          </section>
        ))}
